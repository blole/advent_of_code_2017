...
""".trim()
    ...
    for (line in input.lines()) {
        val regex = Regex("(.*?) \\((.*?)\\)(?: -> (.*))?")
        val (name, weight, children) = regex.matchEntire(line)!!.destructured
        val node = getNode(name)
        node.weight = weight.toInt()
        node.children = children.split(", ").filter { it.isNotEmpty() }.map { getNode(it) }
        node.children.forEach { it.parent = node }
    }

    val root: Node = nodes.entries.first().value.root
    part1 = root.name

    fun <T, C: Comparable<C>> List<T>.oddBy(transform: (T) -> C): T? = when {
        size < 2 -> null
        size == 2 -> if (transform(this[0]) != transform(this[1])) this[0] else null
        else -> {
            val notOdd = transform(subList(0,3).sortedBy { transform(it) }[1])
            this.firstOrNull { transform(it) != notOdd }
        }
    }

    var odd: Node? = root.children.oddBy { it.towerWeight }
    while (odd != null) {
        part2 = odd.parent!!.children.first { it != odd }.towerWeight - odd.subWeight
        odd = odd.children.oddBy { it.towerWeight }
    }
