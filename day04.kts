...
for (passphrase: List<String> in input.lines().map { it.split(" ") }) {
    if (passphrase.count() == passphrase.distinct().count())
        part1++
    val sorted: List<String> = passphrase.map { it.toList().sorted().joinToString("") }
    if (sorted.count() == sorted.distinct().count())
        part2++
}
